...
PROVIDED_TEST("Test recursive power against iterative power") {
     EXPECT_EQUAL(power(7, 3), iterativePower(7, 3));
}
...
STUDENT_TEST("Negative power base test") {
    for (int i = 0; i < 25; i++) {
...
STUDENT_TEST("Negative power exp test") {
    for (int i = 0; i < 25; i++) {
        int base = randomInteger(1, 20);
...
STUDENT_TEST("Negative power exp test") {
    EXPECT_EQUAL(power(5, -2), iterativePower(5, -2));
}

STUDENT_TEST("Compute 0 raised to negative power") {
